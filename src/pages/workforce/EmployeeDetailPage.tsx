    ...
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="sm" asChild>
          <Link to="/workforce/employees">
            ...
            {t('workforce:employees.detail.back')}
          </Link>
        </Button>
      </div>
      ...
      {isLoading && (
        <Card className="p-4">
          ...
          {t('workforce:employees.detail.loading')}
        </Card>
      )}

      {isError && (
        <Card className="p-4 border-error/40 bg-error/10 text-sm text-error">
          {t('workforce:employees.detail.notFound')}
        </Card>
      )}
          ...
          <Card className="p-4 space-y-1">
            ...
            {data.experienceMonths !== undefined && (
              <p className="text-xs text-muted-foreground">
                {t('workforce:employees.detail.experience', {
                  count: data.experienceMonths,
                })}
              </p>
            )}
          </Card>
